...
* 校准逻辑：
*   显示相位 = 用户设置值（0~355°）
*   实际相位 = 显示相位 - phase_offset
*   当示波器显示180°时，长按PA0 → Calibrate记下当前显示值
*   之后 phase_offset = current_display - 180
*   例：旋到示波器180°时OLED显示30° → offset = 30 - 180 = -150
*        设置显示180° → 实际 = 180 - (-150) = 330° → 示波器330° ✓
*        设置显示0°   → 实际 = 0 - (-150) = 150° → 示波器150° ✓
*
* 硬件连接：
*   PA6  ← 输入方波（EXTI双边沿中断）
...
void EdgeSync_Calibrate(float current_display_phase)
{
    // offset = 当前显示值 - 180
    // 之后：实际相位 = 显示值 - offset
    //       当显示值 = 180时 → 实际 = 180 - offset = 180 - (current-180) = 360-current+180 → 不对
    // 重新想：
    //   示波器显示180°时，OLED显示 current_display_phase
    //   说明：actual = current_display_phase - offset = 180
    //   → offset = current_display_phase - 180
