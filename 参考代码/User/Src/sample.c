...
#define RAD_TO_ANGLE(x) ((180.0f/PI)*(x))												//弧度转角度
#define CLAMP_RAD(x) ((x)>PI?(x)-2*PI:((x)<-PI?(x)+2*PI:(x)))		//转到-Π,+Π
    ...
    if (g_sampleState == PHASE_LOCK) {
        ...
        // 使用pid算法实现数字锁相，相位差作反馈，输出差分频率
            ...
            if (g_workMode == PHASE_MODE) {
                ...
                //     g_deltaPhase[1] += 2*PI;
                // }
                g_deltaFreq[0] = pid_calc(&g_phasePid[0], 0, get_delta_rad(g_deltaPhase[0], g_refPhase[0]));	//PID在这里啊
