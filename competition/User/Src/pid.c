...
float pid_calc(pid_struct_t *pid, float ref, float cur){
  ...
  LIMIT_MIN_MAX(pid->i_out, -pid->i_max, pid->i_max);	//将x限制在min和max之间
...
/**
  * @brief  pid calculation with deadband 带死区处理的PID
  * @param  pid    the pid struct containing parameters
  * @param  ref    reference value
  * @param  cur    current value
  * @return pid calculation result
...
/**
  * @brief  pid dual loop calculation 双环PID
